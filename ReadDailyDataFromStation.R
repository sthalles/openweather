...
GetWeatherDataByStation <- function(station.id, summary = FALSE, working.dir = getwd()) {
  ...
  # if the parameter summary is set to TRUE, then, only a summary of the data will be 
  # output with only the columns ID, YEAR, MONTH, ELEMENT, DAY1, DAY2, ... DAYN
  # this option ommit the columns MFLAG1 and QFLAG1 from the output
  if (isTRUE(summary)) {
    columns <- c('ID', 'YEAR', 'MONTH', 'ELEMENT')
    
    for( day in 1:31){
      columns <- c(columns, paste('DAY', day, sep=''))
    }
    
    # subset to get only the desired columns
    daily.data <- daily.data[columns]
  }
...
GetNearestStationData <- function(nearest.stations, element = NULL, working.dir = getwd()) {
  ...
  for (row in 1:nrow(df)) {
    ...
    if (!is.null(element)) {
      ...
      if (nrow(daily.data) == 0)
      {
        warning(paste0("Station: ", station.id, 
                      " does not have data available for these/this kind of element(s): ",
                      paste(element,collapse=" "), ".\n"))
        
        # discard this station and jump to the next one
        next
      }
    }
